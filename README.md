...
## 🚀 最新アップデート (v3.2.4) - テーブル管理システム統合

### 🎯 新機能: `/table` 統合コマンド
- **コマンド統合**: 4つの個別コマンド → 1つの統合コマンドで操作性向上
- **サブコマンド構造**: `setup`, `handout`, `add`, `delete` で論理的グループ化
- **安全な削除機能**: 確認UI付きで誤操作防止、現在カテゴリ自動対象
- **サーバープロフィール対応**: ハンドアウト割り当て時のニックネーム自動採用
...
### 🏗️ テーブル管理 ⭐NEW v3.2.4
- **`/table setup`**: カテゴリ・ロール・チャンネル一括セットアップ
- **`/table handout`**: ハンドアウト割り当て（サーバープロフィール名デフォルト）
- **`/table add`**: 第n陣動的作成（ロール・チャンネル自動生成）
- **`/table delete`**: 安全な確認UI付きカテゴリ完全削除
